...
import datetime
import os
from pricingalgorithm.libs.update_item_dynamodb import update_item
from pricingalgorithm.webservice.generate_rates import generate_rate
from pricingalgorithm.libs.send_html_email import send_html_email
from pricingalgorithm.libs.send_s3 import send_s3
dynamoTable = os.environ['PRICING_ALGO_DB_TABLE']

if os.environ['ENVIRONMENT'] == 'prod':
    from pricingalgorithm.blueyonder.blueyonderquote.prod_config import *
else:
    from pricingalgorithm.blueyonder.blueyonderquote.dev_config import *
...
def blue_yonder(data):
    ...
    try:
        ...
        redshift_data["rejection_code"]="8"
        send_s3(redshift_data)
    ...
    if customer_code not in ['BYCustomer', 'BestBuy']:
        ...
        rejection_payload = json.dumps(rejection_payload)
        redshift_data["rejection_code"]="7"
        send_s3(redshift_data)
        raise Exception("403: " + rejection_payload)
    ...
    try:
        ...
        redshift_data["equipment"]=equipment
        if equipment == 'DryVan':
            equipment = 'VAN'
        ...
        redshift_data["rejection_code"]="2"
        send_s3(redshift_data)
    ...
    try:
        ...
        redshift_data["order_value"]=order_value
    except:
        ...
        rejection_payload = json.dumps(rejection_payload)
        ## TODO: enter all required table data
        redshift_data["rejection_code"]="6"
        send_s3(redshift_data)
        update_item(dynamoTable, message_id, payload=rejection_payload, customer=customer_code,
    ...
    try:
        ...
        rejection_payload = json.dumps(rejection_payload)
        ## TODO: enter all required table data
        redshift_data["rejection_code"]="3"
        send_s3(redshift_data)
        update_item(dynamoTable, message_id, payload=rejection_payload, customer=customer_code,
    ...
    try:
        ...
        redshift_data["pieces"]=pieces
    except:
        pieces = 0
        redshift_data["pieces"]=pieces
    try:
        weight = data['loadTotals']['weight']
        redshift_data["weight"]=weight
    except:
        ...
        rejection_payload = json.dumps(rejection_payload)
        ## TODO: enter all required table data
        redshift_data["rejection_code"]="4"
        send_s3(redshift_data)
        update_item(dynamoTable, message_id, payload=rejection_payload, customer=customer_code,
    ...
    try:
        ...
        redshift_data["teams_required"]=teams_required
    except:
    ...
    try:
        ...
        pickup_city = stop_info[1]['address']['city']
        pickup_state = stop_info[1]['address']['state']
        pickup_country = stop_info[1]['address']['countryCode']
        redshift_data["pickup_address"]=stop_info[1]['address']['streetAddressOne']
        redshift_data["pickup_city"]=stop_info[1]['address']['city']
        redshift_data["pickup_state"]=pickup_state
        redshift_data["pickup_appt_early"]=stop_info[1]['early_arrival']
        redshift_data["pickup_appt_late"]=stop_info[1]['late_arrival']
        redshift_data["pickup_live_handling"]=stop_info[1]['live_handling']
    except:
        ...
        rejection_payload = json.dumps(rejection_payload)
        ## TODO: enter all required table data
        redshift_data["rejection_code"]="5"
        send_s3(redshift_data)
        update_item(dynamoTable, message_id, payload=rejection_payload, customer=customer_code,
    ...
    if number_of_stops > 2:
        ...
        rejection_payload = json.dumps(rejection_payload)
        ## TODO: enter all required table data
        redshift_data["rejection_code"]="1"
        send_s3(redshift_data)
        update_item(dynamoTable, message_id, payload=rejection_payload, customer=customer_code,
        ...
        try:
            if stop_info[1]['address']['countryCode'] == 'USA':
                if stop_info[1]['address']['state'] in valid_states:
                    ...
                    rejection_payload = json.dumps(rejection_payload)
                    redshift_data["rejection_code"]="10"
                    send_s3(redshift_data)
                    update_item(dynamoTable, message_id, payload=rejection_payload, customer=customer_code,
        ...
        try:
            ...
            if len(origin_zip) == 9:
                ...
                pass
            redshift_data["pickup_zip"]=origin_zip
        except:
        ...
        redshift_data["delivery_address"]=stop_info[2]['address']
        redshift_data["delivery_city"]=destination_city
        redshift_data["delivery_state"]=destination_state
        redshift_data["delivery_appt_early"]=stop_info[2]['early_arrival']
        redshift_data["delivery_appt_late"]=stop_info[2]['late_arrival']
        redshift_data["delivery_live_handling"]=stop_info[2]['live_handling']
        ...
        try:
            ...
            if len(destination_zip) == 9:
                ...
                pass
            redshift_data["delivery_zip"]=destination_zip
        except:
            # google_key = 'AIzaSyDod39KtNuE6ufiFSLrpC83hK48jwCka1A'
        ...
        redshift_data["requested_cost_type"]=requested_cost_type
        ...
        if rate_output['rate'] == 'Error in rate calculation':
            ...
            rejection_payload = json.dumps(rejection_payload)
            redshift_data["rejection_code"]="11"
            send_s3(redshift_data)
            update_item(dynamoTable, message_id, payload=rejection_payload, customer=customer_code,
        ...
        if requested_cost_type == "AllIn":
            ...
            rejection_payload = json.dumps(rejection_payload)
            redshift_data['rejection_code']="9"
            send_s3(redshift_data)
            update_item(dynamoTable, message_id, payload=rejection_payload, customer=customer_code,
        ...
        ## TODO: enter all required table data
        redshift_data["rate"]=rate
        redshift_data["quote_status"]='Accepted'
        redshift_data["rejection_code"]="0"
        send_s3(redshift_data)
